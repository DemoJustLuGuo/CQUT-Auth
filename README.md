...
| 环境变量                                                   |   默认值 | 说明                                   |
| :--------------------------------------------------------- | -------: | :------------------------------------- |
| `OIDC_MANAGEMENT_CLIENT_MAX_PER_PROJECT`                   |     `10` | 每个项目的非停用客户端上限。           |
| `OIDC_MANAGEMENT_CLIENT_MAX_PENDING_PER_PROJECT`           |      `5` | 每个项目的 Pending Revision 上限。     |
| `OIDC_MANAGEMENT_CLIENT_MAX_PER_SUBJECT`                   |     `30` | 项目创建者跨项目的非停用客户端上限。   |
| `OIDC_MANAGEMENT_CLIENT_MAX_PENDING_PER_SUBJECT`           |     `15` | 项目创建者跨项目的 Pending 上限。      |
| `OIDC_MANAGEMENT_CLIENT_QUOTA_ADMIN_EXEMPT`                |   `true` | 管理员是否豁免以上配额。               |
| `OIDC_MANAGEMENT_PROJECT_MAX_ACTIVE_PER_SUBJECT`           |      `5` | 每个 Subject 的 active 项目上限。      |
| `OIDC_MANAGEMENT_PROJECT_QUOTA_ADMIN_EXEMPT`               |   `true` | 管理员是否豁免项目配额和创建限流。     |
| `OIDC_MANAGEMENT_PROJECT_CREATE_RATE_LIMIT_SUBJECT_MAX`    |      `3` | 单个 Subject 在窗口内的项目创建上限。  |
| `OIDC_MANAGEMENT_PROJECT_CREATE_RATE_LIMIT_IP_MAX`         |     `10` | 单个来源 IP 在窗口内的项目创建上限。   |
| `OIDC_MANAGEMENT_PROJECT_CREATE_RATE_LIMIT_WINDOW_SECONDS` |   `3600` | 项目创建限流窗口秒数。                 |
